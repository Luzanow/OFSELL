...
from aiogram.types import InputMediaPhoto, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, KeyboardButton
...
GROUP_ID = -1001234567890  # заміни на ID групи
...
async def finish_application(message):
    ...
    for item in data["items"]:
        for pid in item["photos"]:
            if len(media) == 0:
                media.append(InputMediaPhoto(pid, caption="Фото до заявки"))
            else:
